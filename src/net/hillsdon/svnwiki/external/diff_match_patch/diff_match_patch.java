package net.hillsdon.svnwiki.external.diff_match_patch;
...
public class diff_match_patch {
  ...
  public String diff_prettyHtml(LinkedList<Diff> diffs) {
    ...
    for (Diff aDiff : diffs) {
      ...
      if (aDiff.operation == Operation.DELETE) {
        html.append("<DEL STYLE=\"background:#FFE6E6;\" TITLE=\"i=" + aDiff.index
                + "\">" + text + "</DEL>");
  ...
  public String diff_toDelta(LinkedList<Diff> diffs) {
    ...
    return txt.toString().replace("%20", " ");
  }
    ...
    for (String token : tokens) {
      ...
      switch(token.charAt(0)) {
        ...
        case '=':
          ...
          String text = text1.substring(pointer, pointer += n);
          if (token.charAt(0) == '=') {
        ...
        default:
          ...
          assert false : "Invalid diff operation in diff_fromDelta()";
      }
    }
    assert pointer == text1.length() : "Text length mismatch in diff_fromDelta()";
    return diffs;
  }
...
class Patch {
  ...
  public String toString() {
    ...
    String txtString = txt.toString()
             .replace("%3D", "=").replace("%3B", ";").replace("%27", "'")
