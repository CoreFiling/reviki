...
public class TestRenderingExtensions extends JsonDrivenRenderingTest {
  ...
  protected String render(final String input) throws IOException, PageStoreException {
    SvnWikiRenderer renderer = new SvnWikiRenderer(config, new SimplePageStore(), new InternalLinker(new ExampleDotComWikiUrls()), Suppliers.ofInstance(Collections.<Macro> emptyList()));
